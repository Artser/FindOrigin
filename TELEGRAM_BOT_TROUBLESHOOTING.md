# Устранение неполадок Telegram-бота

## Проблема: Бот не отвечает на сообщения

Если на сайте все работает, но бот не отвечает, проверьте следующее:

## Проверка 1: Webhook настроен

### Проверка статуса webhook

```powershell
# Замените YOUR_BOT_TOKEN на ваш токен из .env.local
$token = "YOUR_BOT_TOKEN"
$response = Invoke-RestMethod -Uri "https://api.telegram.org/bot$token/getWebhookInfo"
$response | ConvertTo-Json
```

**Что проверить:**
- `url` - должен быть ваш webhook URL (например, `https://ваш-домен.vercel.app/api/webhook` или `https://ваш-ngrok-url.ngrok.io/api/webhook`)
- `pending_update_count` - должно быть 0 (если есть ошибки, будет > 0)
- `last_error_message` - если есть ошибки, здесь будет описание

### Установка webhook

**Для локальной разработки (через ngrok):**

1. Запустите ngrok в отдельном терминале:
   ```powershell
   ngrok http 3000
   ```

2. Скопируйте URL (например: `https://abc123.ngrok.io`)

3. Установите webhook:
   ```
   http://localhost:3000/api/set-webhook?url=https://abc123.ngrok.io/api/webhook
   ```

**Для продакшена (Vercel):**

1. Деплойте проект на Vercel
2. Установите webhook:
   ```
   https://ваш-домен.vercel.app/api/set-webhook?url=https://ваш-домен.vercel.app/api/webhook
   ```

## Проверка 2: Логи сервера

Проверьте терминал, где запущен `npm run dev`. При отправке сообщения боту должны быть логи:

**Нормальные логи:**
```
POST /api/webhook 200 in 123ms
Ошибка при асинхронной обработке запроса: [если есть ошибка]
```

**Если нет логов:**
- Webhook не настроен или не работает
- Telegram не может достучаться до вашего сервера

## Проверка 3: Ошибки в логах

### Типичные ошибки:

**"TELEGRAM_BOT_TOKEN не установлен"**
- Проверьте `.env.local` - должен быть `TELEGRAM_BOT_TOKEN`
- Перезапустите сервер

**"Telegram API error: 401"**
- Неправильный токен бота
- Проверьте токен на https://t.me/BotFather

**"Telegram API error: 403"**
- Бот заблокирован или удален
- Проверьте статус бота

**"Ошибка при асинхронной обработке запроса"**
- Ошибка в процессе обработки
- Проверьте детали ошибки в логах
- Проверьте настройки поисковых API

## Проверка 4: Тестирование команд

### Команда /start

Отправьте боту команду `/start`. Бот должен ответить приветствием.

**Если не отвечает:**
- Проверьте логи сервера
- Проверьте, что webhook настроен
- Проверьте, что сервер запущен

### Обычное сообщение

Отправьте боту любой текст. Бот должен:
1. Ответить "🔍 Анализирую ваш запрос..."
2. Затем "🌐 Ищу возможные источники..."
3. И так далее

**Если не отвечает:**
- Проверьте логи сервера на наличие ошибок
- Проверьте настройки поисковых API

## Проверка 5: Обработка ошибок

Код обновлен для отправки сообщений об ошибках пользователю. Если происходит ошибка:
- Пользователь должен получить сообщение с описанием ошибки
- В логах сервера будет детальная информация

## Быстрая диагностика

1. **Проверьте webhook:**
   ```powershell
   $token = "YOUR_BOT_TOKEN"
   Invoke-RestMethod -Uri "https://api.telegram.org/bot$token/getWebhookInfo"
   ```

2. **Проверьте логи сервера:**
   - Отправьте сообщение боту
   - Проверьте терминал с `npm run dev`
   - Ищите ошибки (красным цветом)

3. **Проверьте переменные окружения:**
   - `TELEGRAM_BOT_TOKEN` должен быть установлен
   - Перезапустите сервер после изменения `.env.local`

4. **Проверьте доступность сервера:**
   - Для локальной разработки: используйте ngrok
   - Для продакшена: убедитесь, что домен доступен

## Если ничего не помогает

1. **Проверьте все логи** (сервер, Vercel, Telegram)
2. **Убедитесь, что все переменные окружения установлены**
3. **Перезапустите сервер**
4. **Переустановите webhook**

## Полезные команды

```powershell
# Проверка статуса webhook
$token = "YOUR_BOT_TOKEN"
Invoke-RestMethod -Uri "https://api.telegram.org/bot$token/getWebhookInfo"

# Удаление webhook (если нужно начать заново)
Invoke-RestMethod -Uri "https://api.telegram.org/bot$token/deleteWebhook"

# Проверка работы бота
Invoke-RestMethod -Uri "https://api.telegram.org/bot$token/getMe"
```



