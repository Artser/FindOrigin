...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (text?.startsWith('/')) {
      ...
      if (command === '/start') {
        console.log('[WEBHOOK] Обработка команды /start для chatId:', chatId);
        try {
          // Импортируем sendMessage динамически, чтобы избежать циклических зависимостей
          const { sendMessage } = await import('@/lib/telegram');
          console.log('[WEBHOOK] sendMessage импортирован, отправляем приветственное сообщение...');
          ...
          console.log('[WEBHOOK] Приветственное сообщение успешно отправлено');
          return NextResponse.json({ ok: true });
        } catch (error) {
          console.error('[WEBHOOK] Ошибка при отправке приветственного сообщения:', error);
      ...
      if (command === '/help') {
        console.log('[WEBHOOK] Обработка команды /help для chatId:', chatId);
        try {
          const { sendMessage } = await import('@/lib/telegram');
          await sendMessage({
            chatId,
          ...
          console.log('[WEBHOOK] Справка успешно отправлена');
          return NextResponse.json({ ok: true });
        } catch (error) {
          console.error('[WEBHOOK] Ошибка при отправке справки:', error);
          return NextResponse.json({ ok: false, error: 'Failed to send help message' });
        }
      }
    }
